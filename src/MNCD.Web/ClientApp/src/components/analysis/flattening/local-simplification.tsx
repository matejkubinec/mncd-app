...
class LocalSimplification extends React.Component<ReduxProps, IState> {
  ...
  handleApplyRelevanceAll = () => {
    const rel = this.props.relevances.map(() =>
      Number(this.state.relevanceAll)
    );
  ...
  renderRows = () => {
    return this.props.relevances.map((rel, i) => {
      ...
      const borderBottomColor =
        Number(rel) >= Number(this.props.treshold) ? over : under;
      const onChange = (_: any, v: string | undefined) =>
        this.handleRelevanceChange(i, v);
...
const mapProps = (state: RootState) => {
  ...
  const {
    ...
    relevances,
    weightEdges
  } = request.flatteningAlgorithmParameters;
  return {
